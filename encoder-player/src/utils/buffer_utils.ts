export async function byobReader(reader: ReadableStreamBYOBReader, buffer: ArrayBufferLike, offset: number, size: number): Promise<ArrayBuffer> {
  if (size <= 0) {
    return buffer;
  }
...
export async function readUntilEof (readableStream: ReadableStream, blockSize: number) {
  const chunkArray = []
  let totalLength = 0
